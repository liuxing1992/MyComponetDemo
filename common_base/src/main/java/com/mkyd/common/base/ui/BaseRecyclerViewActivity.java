...
public abstract class BaseRecyclerViewActivity<T, P extends BaseMvpPresent> extends BaseUIActiivty<P> implements IRecyclerView<T>
    ...
    public void initView() {
        ...
        if (getReHeaderView() != 0) {
            mAdapter.addHeaderView(LayoutInflater.from(this).inflate(getContentViewId(), null, false));
        }
        if (getReFooterView() != 0) {
            mAdapter.addFooterView(LayoutInflater.from(this).inflate(getContentViewId(), null, false));
        }
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if (newState == RecyclerView.SCROLL_STATE_IDLE) {
                    //静止没有滚动
                    Glide.with(getActiivty()).resumeRequests();
                } else if (newState == RecyclerView.SCROLL_STATE_DRAGGING) {
                    //正在被外部拖拽,一般为用户正在用手指滚动
                    Glide.with(getActiivty()).pauseRequests();
    ...
    public void refreshUI(List<T> data) {
        ...
        if (data != null) {
            ...
            if (data.size() == 0) {
                //数据空
                if (getReHeaderView() == 0 && getReFooterView() == 0) {
                    showEmptyLayout("");
                }
            } else {
                //数据不为空
