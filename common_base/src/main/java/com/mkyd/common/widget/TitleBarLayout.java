package com.mkyd.common.widget;
...
public class TitleBarLayout extends ViewGroup implements View.OnClickListener {
    ...
    int titleRightIds[]= {17201341,17201342,17201343,17201344,17201345};
    ...
    private int mActionTextColor=0xff656565;
    private int mHeight;

    private int actionTextSize = 15;
    ...
    private void initView(Context context) {
        ...
        mLeftText.setId(R.id.title_bar_left);
    ...
    /**
     * Adds a new {@link Action} at the specified index.u
     * @param action the action to add
...
//            params.setMargins(SizeUtils.dp2px(10),0, 0,0);
            params.setMargins(0,0, 0,0);//不需要间距
        }
        View view = inflateAction(action,index);
    ...
    public View getActionViewByIndex(int index){
     ...
     * @return a view
     */
    private View inflateAction(Action action,int index) {
        View view = null;
        if (TextUtils.isEmpty(action.getText())) {
            ImageView img = new ImageView(getContext());
            img.setImageResource(action.getDrawable());
            img.setAlpha(action.getAlpha());
            if (titleRightIds.length>index) {
                img.setId(titleRightIds[index]);
            }else {
                img.setId(index);
            }
            view = img;
            ...
            text.setTextSize(actionTextSize);
            if (titleRightIds.length>index) {
                text.setId(titleRightIds[index]);
            }else {
                text.setId(index);
            }
            if (mActionTextColor ==-1) {
//                text.setShadowLayer(3,3,3, Color.parseColor("#88000000"));
            }
    ...
    public static abstract class ImageAction implements Action {
        ...
        public ImageAction(int drawable,float alpha) {
            mDrawable = drawable;
            this.alpha = alpha;
        }
